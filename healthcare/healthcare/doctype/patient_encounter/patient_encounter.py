...
class PatientEncounter(Document):
	...
	def validate_medications(self):
		...
		for item in self.drug_prescription:
			if not item.medication and not item.drug_id:
				frappe.throw(
...
@frappe.whitelist()
def create_material_issue_stock_entry(doc):
    ...
    for drug_prescription in doc.drug_prescription:
        stock_entry.append("items", {
            "item_code": drug_prescription.drug_id,
            "qty": drug_prescription.precribed_quantity,  # Check the spelling of 'precribed_quantity'
            # "uom": "Your UOM",  # Replace with the actual unit of measure
            "conversion_factor": 1,  # Replace with the actual conversion factor
